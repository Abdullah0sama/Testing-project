...
public class SubjectGrades {
    ...
    public void setSubjectCode(String subjectCode) {
        if(subjectCode.length() != 7 && subjectCode.length() != 6){
            System.out.println("Subject code should be 6 or 7 letters");
        }
        String alpaCode = subjectCode.substring(0,2);
        String numCode = subjectCode.substring(3,5);
        if(!alpaCode.matches("[a-zA-Z]+")){
            System.out.println("The first 3 letters should be alphabet");
        } else if (!numCode.matches(("[0-9]+"))) {
            System.out.println("From fourth to Sixth letters should be numbers");
        }else if ((subjectCode.length() == 7) && (subjectCode.indexOf("s") != 7 )){
            System.out.println("The seventh letter should be (S)");
        }
    ...
    public void setStudentID(String studentID) {
       if (!(studentID.length() == 8 && studentID.substring(0, 7).matches("\\d+"))) {
            System.out.println("The first seven characters are not all numbers.");
        }
        StudentID = studentID;
    }
    ...
    public void setMidterm(int midterm) {
        if (!(midterm >= 0 && midterm <= 20)) {
            System.out.println("Mark must be between 0 and 20.");
        }
        Midterm = midterm;
    }
    ...
    public void setFinal(int aFinal) {
        if (!(aFinal >= 0 && aFinal <= 60)) {
            System.out.println("Mark must be between 0 and 60.");
        }
        Final = aFinal;
    }
